...
## ... Files
l.file <- list(
  gf_raw = path(l.path$data, "gf_original/regression_vars.csv"),
  gf_cln = path(l.path$data, "gf_original/regressions_ready_data3.dta"),
...
df.gf_cln <- read_dta(l.file$gf_cln) %>%
  ...
  # filter(!(plant_code==6639 & boiler_id=="G2")) %>%
  # filter(!(plant_code==6823 & boiler_id=="W1")) %>%
  # filter(!(plant_code==10495 & boiler_id=="CRECV")) %>%
  # filter(!(plant_code==50397 & boiler_id=="5PB036")) %>%
  # filter(!(plant_code==50407 & boiler_id=="7PB")) %>%
  # filter(!(plant_code==50481 & boiler_id=="325-31")) %>%
  # filter(!(plant_code==57919 & boiler_id=="09")) %>%
  select(year,
         states,
         ORISPL = plant_code,
         BOILERID = boiler_id,
...
ggplot() +
  ...
  scale_color_viridis_c(direction=-1, begin=0.1, end=0.85, alpha=0.4) +
  scale_size(breaks=c(5,10)) +
  coord_sf(crs=sf::st_crs(2163)) +
  labs(#title="Coal-fired power plants",
       color="Share NSR\ngrandfathered",
...
# ggsave(path(l.path$out, date, "fig.gf_share_map.pdf"),
...
tbl_footer <- c("\\hline\\hline",
                paste0("\\multicolumn{4}{p{14cm}}{\\textit{Notes}:  This table displays average ",
...
f_header %>% write_lines(path(l.path$out, date, "tbl.balance.tex"), append=FALSE)
tbl_header %>% write_lines(path(l.path$out, date, "tbl.balance.tex"), append=TRUE)
tbl_coef %>% write_tsv(path(l.path$out, date, "tbl.balance.tex"), 
                       append=TRUE, col_names=FALSE, escape="none")
tbl_midder %>% write_lines(path(l.path$out, date, "tbl.balance.tex"), append=TRUE)
tbl_summary %>% write_tsv(path(l.path$out, date, "tbl.balance.tex"), 
                          append=TRUE, col_names=FALSE, escape="none")
tbl_footer %>% write_lines(path(l.path$out, date, "tbl.balance.tex"), append=TRUE)
```
...
if (knitr::is_html_output()) {
  df.bal %>%
    kable(format="html", align="lrrr", booktabs=TRUE, linesep=c(""),
          caption="Average characteristics of boilers by local regulation < 1.1",
...
tbl_header <- c("\\begin{center}",
                ...
                "\\\\",
                "\\hline\\hline",
                paste0("\\textbf{Variable} & \\multicolumn{1}{c}{\\textbf{Local regulation < 1.1}} & ",
                       "\\multicolumn{1}{c}{\\textbf{Local regulation $\\geq$ 1.1}} & ",
                       "\\multicolumn{1}{c}{\\textbf{Difference}} \\\\ [1.0ex]"),
                "\\midrule",
                "\\endfirsthead",
                "\\hline\\hline",
...
tbl_footer <- c("\\hline\\hline",
                paste0("\\multicolumn{4}{p{14cm}}{\\textit{Notes}:  This table displays average ",
                       "characteristics by local regulation at a cutoff of 1.1 [units?].  Standard deviations are in ",
                       "parentheses, with \\textit{t}-statistics of the difference between ",
                       "`grandfathered' and `non-grandfathered' boilers in brackets where ",
...
## Output TeX table file
f_header %>% write_lines(path(l.path$out, date, "tbl.balance_lr11.tex"), append=FALSE)
tbl_header %>% write_lines(path(l.path$out, date, "tbl.balance_lr11.tex"), append=TRUE)
tbl_coef %>% write_tsv(path(l.path$out, date, "tbl.balance_lr11.tex"), 
                       append=TRUE, col_names=FALSE, escape="none")
tbl_midder %>% write_lines(path(l.path$out, date, "tbl.balance_lr11.tex"), append=TRUE)
tbl_summary %>% write_tsv(path(l.path$out, date, "tbl.balance_lr11.tex"), 
                          append=TRUE, col_names=FALSE, escape="none")
tbl_footer %>% write_lines(path(l.path$out, date, "tbl.balance_lr11.tex"), append=TRUE)
```
...
df.boiler12 <- df.boiler |>
  mutate(LR = ifelse(LOCALREG<1.2, 1, 0))

df.bal_n <- df.boiler12 %>%
  group_by(LR) %>%
  summarise(N = n(), .groups="drop") %>%
  mutate(N = formatC(N, format="f", digits=0, big.mark=",")) %>%
  pivot_wider(names_from=LR, names_prefix="LR_", values_from="N") %>%
...
if (knitr::is_html_output()) {
  df.bal %>%
    kable(format="html", align="lrrr", booktabs=TRUE, linesep=c(""),
          caption="Average characteristics of boilers by local regulation < 1.2",
          col.names=c("","Local regulation < 1.2","Local regulation >= 1.2.","Difference")) 
} else {
  df.bal
}
```
...
tbl_header <- c("\\begin{center}",
                ...
                "\\\\",
                "\\hline\\hline",
                paste0("\\textbf{Variable} & \\multicolumn{1}{c}{\\textbf{Local regulation < 1.2}} & ",
                       "\\multicolumn{1}{c}{\\textbf{Local regulation $\\geq$ 1.2}} & ",
                       "\\multicolumn{1}{c}{\\textbf{Difference}} \\\\ [1.0ex]"),
                "\\midrule",
                "\\endfirsthead",
                "\\hline\\hline",
...
tbl_footer <- c("\\hline\\hline",
                paste0("\\multicolumn{4}{p{14cm}}{\\textit{Notes}:  This table displays average ",
                       "characteristics by local regulation at a cutoff of 1.2 [units?].  Standard deviations are in ",
                       "parentheses, with \\textit{t}-statistics of the difference between ",
                       "`grandfathered' and `non-grandfathered' boilers in brackets where ",
                       "*** p<0.001;  ** p<0.01;  * p<0.05.  All variables utilize the full extent ",
                       "of our dataset, except for those from CEMS which are from 1997 onwards and ",
...
## Output TeX table file
f_header %>% write_lines(path(l.path$out, date, "tbl.balance_lr12.tex"), append=FALSE)
tbl_header %>% write_lines(path(l.path$out, date, "tbl.balance_lr12.tex"), append=TRUE)
tbl_coef %>% write_tsv(path(l.path$out, date, "tbl.balance_lr12.tex"), 
                       append=TRUE, col_names=FALSE, escape="none")
tbl_midder %>% write_lines(path(l.path$out, date, "tbl.balance_lr12.tex"), append=TRUE)
tbl_summary %>% write_tsv(path(l.path$out, date, "tbl.balance_lr12.tex"), 
                          append=TRUE, col_names=FALSE, escape="none")
tbl_footer %>% write_lines(path(l.path$out, date, "tbl.balance_lr12.tex"), append=TRUE)
```
