...
save "$path_use_data\electricity_demand_by_state.dta", replace

*************************************************************************************
*** Get improved ownership assignment  ***
*************************************************************************************
import delimited "$path_use_data\utils_2007-2018.csv", varnames(1)  clear
keep if util_entity_type!="NA"
drop util_attn_line	v1	util_pobox util_street_address	util_city	util_state	util_zip util_zip4	util_contact_firstname	util_contact_lastname	util_contact_title	util_contact2_firstname	util_contact2_lastname	util_contact2_tile util_street_address2	util_owner	util_operator	util_asset_manager	util_other_relationships_plants
drop if utility_code==.
drop if year==.
*C - co-op, Q - Independent Power Producer, I- Investor owned utility, P- political, F - fed, S- state. Ind-industrial, Com - commercial, M - municipial
save "$path_use_data\utility_data.dta", replace
...
*************************************************************************************
clear
import excel "$path_use_data\gf_status_born_around_1978",  firstrow
keep plant_code boiler_id gf
rename gf pre_1978
drop if plant_code==.
save "$path_use_data\gf_status_born_around_1978.dta", replace


*************************************************************************************
*** Get additional info on independent power producers
*** data is from EIA 867 available for years 1989 - 1998
*************************************************************************************
clear
...
*** data is from EIA 923 and 423 
*************************************************************************************

clear
import delimited "$path_data\sulfur_iv.csv"
rename orispl plant_code
save "$path_use_data\sulfur_iv.dta", replace
...
*************************************************************************************
clear
import delimited "$path_use_data\ARP_power_plants.csv", clear 
gen ARP_subject=1
save "$path_use_data\ARP_PhaseI.dta", replace
...
*************************************************************************************
clear
import excel "$path_use_data\ARP_prices.xlsx", sheet("Data") firstrow
save "$path_use_data\ARP_prices.dta", replace
...
*************************************************************************************
clear
import delimited "$path_use_data\coal_char_data_by_plant_year.csv", clear 
drop v1 mine_cou*
save "$path_use_data\coal_trades_data.dta", replace
...
*************************************************************************************
 ...
 rename YEAR year
 save "$path_use_data\cems_yearly.dta", replace
 ...
 *For some states the price data is missing. Use the country-wide average
 ...
 save "$path_use_data\coal2gasPrice.dta", replace
...
*************************************************************************************
clear
import delimited "$path_use_data\eia_capacity.csv", clear 
states_def
...
save "$path_use_data\State_capacity.dta", replace
...
clear
import delimited "$path_use_data\regression_vars.csv", clear
...
gen_planned_repower_y gen_topping_bottoming gen_winter_cap_mmbtu ///
water_source fgd_trains_100per  gypsum_ponds gypsum_landfill ///
gypsum_onsite gypsum_sold gypsum_offsite other_byproduct_total other_byprod_landfill other_byprod_ponds ///
other_byprod_onsite other_byprod_sold other_byprod_offsite exp_fly_ash_collection exp_fly_ash_disposal ///
exp_bott_ash_collection exp_bott_ash_disposal exp_fgd_collection exp_water_abate_coll ///
exp_water_abate_disp exp_other_collection exp_other_disposal exp_other_other useful_therm_directheat ///
useful_therm_spaceheat useful_therm_processheat useful_therm_enduser ///
...
fuel7_is_coal waste_produced ///
gen_planned_uprates_net_winter_c gen_planned_derates_net_winter_c gen_uprate_derate_completed_this ///
pm_ctrl_noncomp1 pm_ctrl_noncomp2 pm_ctrl_noncomp3 nox_period pm_comp_y nox_comp_y nox_std_rate nox_unit ///
pm_unit pm_std_rate pm_period nox_ctrl_noncomp1 nox_ctrl_noncomp2 nox_ctrl_noncomp3 byprod_rev_fly_ash ///
byprod_rev_bott_ash byprod_rev_fly_bott_ash  byprod_rev_other byprod_rev_total ///
new_exp_abate_water new_exp_abate_waste new_exp_other_abate new_exp_air_abate fly_ash_total nerc naics ///
petro_heat_yr_plus_5 petro_heat_yr_plus_10 petro_sulfur_yr_plus_5 petro_sulfur_yr_plus_10 hg_emission_ot ///
air_abate3yr boiler_quant10yr sludge_pond alt_fuel_usage_30_days alt_fuel_days_to_switch ///
year_alt_fuel_last_used primary_fuel8 primary_fuel6 ann_ash_content plt_fuel_sc plt_fuel_sub plt_fuel_wc plt_fuel_lig ///
 plt_st_address exp_total_collection exp_total_disposal exp_total_other fire_primary_fuel3 ///
air_flow_100_pct_load alt_fuel_code3 alt_fuel_code2 fuel6_is_coal fuel5_is_coal fuel4_is_coal fuel3_is_coal fuel2_is_coal fuel1_is_coal ///
fgd_cost_total_by_plt_tot fgd_tot_cost_structure_by_boiler fgd_tot_cost_disposal_by_boiler fgd_tot_cost_other_by_boiler ///
fgd_tot_expend_feed_by_boiler fgd_tot_cost_total_by_boiler fgd_tot_expend_labor_by_boiler ///
fgd_tot_expend_disposal_by_boile fgd_tot_expend_other_by_boiler fgd_tot_expend_total_by_boiler reg_so2_ppm ///
fgd_costinc5yr fgd_costinc10yr fgd_costinc3yr fgd_cost10yr fgd_cost5yr fgd_cost3yr fgd_costinc1yr ann_heat_content so2_std_percent_scrubb efficiency_50_pct_load
...
*for some funny reason the capacity we have from EIA seems to be inflated up, with individual boilers being over 3GW
*when comparing with capacity data from the EIA-EPA crosswalk, it seems like the inflation factor is 3.34
...
replace leg_lbs_mmbtu_calc=leg_lbs_mmbtu_calc[_n-1] if leg_lbs_mmbtu_calc>9.09 & year==2006 & leg_lbs_mmbtu_calc[_n-1]==leg_lbs_mmbtu_calc[_n+1] & year[_n+1]==2007 & year[_n-1]==2005
replace leg_lbs_mmbtu_calc=leg_lbs_mmbtu if leg_lbs_mmbtu_calc==. & leg_lbs_mmbtu!=.
replace leg_lbs_mmbtu_calc=leg_lbs_mmbtu_calc[_n-1] if year==2006 & leg_lbs_mmbtu_calc==. & leg_lbs_mmbtu_calc[_n-1]!=leg_lbs_mmbtu_calc[_n+1] & year[_n+1]==2007 & year[_n-1]==2005


*******************************************************************
******** GENERATE THE SURVIVAL/NEW INDICATOR **********************
*******************************************************************
...
*******************************************************************
************ ADD ELEVTRICITY DEMAND BY STATE AND UTILITY DATA 
*******************************************************************************
joinby year plt_state using "$path_use_data\electricity_demand_by_state.dta", unmatched(master)
drop _merge

destring utility_code, replace force
joinby utility_code year using "$path_use_data\utility_data.dta", unmatched(both)
*some 3.5 ths obs do not have utility code
*36 ths obs not matched to the code but the file only for years above 2013. For those years - only 86 unmatched
*the EIA 860 data has, on the other hand, 21 ths obs that are not matched to our data - could be owners of gas, solar etc
drop if _merge==2
drop _merge 
sort utility_code year util_entity_type 
replace util_entity_type=util_entity_type[_n+1] if util_entity_type=="" & util_entity_type[_n+1]!="" & utility_code==utility_code[_n+1] & (year==year[_n+1]-1|year==year[_n+1])
...
gen type_check=commerc_ind+IOU+other
 sort plant_code boiler_id year
 replace ut_type=ut_type[_n-1] if ut_type==. & ut_type[_n-1]!=. & plant_code==plant_code[_n-1] & boiler_id==boiler_id[_n-1]  & type_check==1
 ...
 *unclear gf status
 ...
 drop ut_type_aux* commerc_ind_aux commerc_ind IOU type_check other_aux other util_entity_type no_types nvals type


joinby year plant_code using "$path_use_data\nonutilityData.dta", unmatched(master)
*all merged observations have no utility type
replace ut_type=Sector if _merge==3 & ut_type==.
drop _merge Sector

*******************************************************************
*** GENERATE THE NSR & NSPS GRANDFATHERING INDICATORS 
*******************************************************************

*NSR is for boilers for which construction began after September 18, 1978 and above 73MW. 
*pre_NSR gathers hand-picked data on the beginning of construction for boilers
*where the inservice date is relatively shortly after 1978

order plant_code boiler_id year inservice_y type_of_boiler nsr_d so2_unit so2_std_rate nsr_permit_y nsr_first_permit_y nsr_permit_num reg_so2_lbs_mmbtu leg_lbs_mmbtu so2_regulation so2_std_rate inservice_m  inservice_y    reported_so2_lbs_mmbtu leg_lbs_mmbtu_calc leg_lbs_mmbtu_calc

*NSR is for bigger boilers but some boilers don't have capacity info in some years. Below we are dealing with most cases where that info is missing 
...
replace capacity_at_begin=capacity_at_begin[_n-1] if (capacity_at_begin==. & plant_code==plant_code[_n-1] & boiler_id==boiler_id[_n-1])


*Generate the time-invariant grandfathering status
*Import data from Aaron with boiler classification around 1978
joinby plant_code boiler_id using "$path_use_data\gf_status_born_around_1978.dta", unmatched(master)
*The rules for NSR were initially introduced only for boilers operated
* "for the purpose of supplying more than one-third of its potential electric output capacity 
* and more than 25 MW net-electrical output to any utility power distribution system for sale"  (40 CFR 60.41Da)
*Update for separate rules for commercial and industrial boilers (subpart Db) - boilers greater than 100MMBtu that were also built after 1984 (e.g. grandfathering applies). (40 CFR 60.40Db et seq.)
*  that industrial-commercial-institutional boilers built before 1984 but after 1971 that are over 250MMBtu are still subject to the NSPS standard of 1.2 lbs/MMBtu (Subpart D). 
*a separate set of rules for small industrial-commercial-institutional boilers (greater than 10MMBtu but less than 100MMBtu) where construction commenced after June 9, 1989 (again, grandfathering applies).
* These regulations include a 90% reduction standard and a performance standard of 0.20 lb/MMBtu.
*  Using typical boiler and generating efficiencies, 100 MMBtu/hr heat input is equal to about 10 MW electric output.
...
*C - co-op (1), Q - Independent Power Producer (8), I- Investor owned utility (4), P- political (7), F - fed (3), S- state (9). Ind-industrial (5), Com - commercial (2), M - municipial (6)
...
*******************************************************************
*** MERGE THE COAL DATA and ARP PRICES 
*******************************************************************
joinby year plant_code using "$path_use_data\coal_trades_data.dta", unmatched(master)
drop _merge fuel_cost_* heat_content_* quantity_received_*
destring sulfur_content_tot, replace force

joinby year  using "$path_use_data\ARP_prices.dta", unmatched(master)

*CAIR required that 2 permits required for one unit of emissions. Thus, double prices
*in SO2-CAIR subject states:
*In Alabama, District of Columbia, Florida, Georgia, Illinois, Indiana, Iowa, Kentucky, Louisiana, Maryland, Michigan, Minnesota, Mississippi, Missouri, New York, North Carolina, Ohio, Pennsylvania, South Carolina, Tennessee, Texas, Virginia, West Virginia, and Wisconsin
...
joinby plant_code year  using "$path_use_data\ARP_PhaseI.dta", unmatched(master)
...
***********************************************************
joinby year plt_state using "$path_use_data\State_capacity.dta" , unmatched(master)
drop _merge
...
*********************************************
joinby year plant_code boiler_id using "$path_use_data\cems_yearly.dta", unmatched(master)
drop _merge

joinby plant_code boiler_id using "$path_use_data\capacity_data_from_CEMS.dta", unmatched(master)
drop _merge
...
**************************************************
joinby year plt_state using "$path_use_data\coal2gasPrice.dta", unmatched(master)
drop _merge
...
****** sulfur_net - sulfur content of coal weighted by distance over the network 
*********************************************
joinby plant_code  using "$path_use_data\sulfur_iv.dta", unmatched(master)
drop _merge
...
***************************************************************
**********Generate additional variables for regressions *******
********** anticipation, boiler manuf etc   *******************
***************************************************************
...
so2_proposed_ctrl3  so2_proposed_ctrl2 nox_existing_ctrl1 nox_existing_ctrl2 ///
 ...
 cogen_type cogen_useful_thermal coal_heat_yr_plus_10 pm_regulation nox_regulation NOX_MASS ///
 coal_heat_yr_plus_5 coal_sulfur_yr_plus_5 coal_sulfur_yr_plus_10 utility_flag97_00 plt_anngen ///
 ifnew_lbs_so2_hr ifnew_calc_plt_lbs_so2_hr ifnew_linked_state_reg plt_zip plt_fuel_bit so2_existing_ctrl3 ///
 so2_existing_ctrl2 so2_ctrl_noncomp3 so2_ctrl_noncomp2 primary_fuel5 primary_fuel4 primary_fuel3 primary_fuel2 ///
  so2_proposed_ctrl1 nsr_permit_m  steam_plt_type plt_inservice_m ///
 fgd_avg_energy_consump_kwh_by_bo fgd_tot_energy_consump_kwh_by_bo fgd_avg_eff_sulfur_at_100_by_boi /// 
 fgd_tot_so2_emission_rate_by_boi fgd_earliest_inservice_by_boiler gen_summer_cap_mmbtu ///
 other_fire_steam_flow retirement_m plt_anncon plt_annstk plt_fuel_col ///
  boiler_geninc boiler_quantinc3yr boiler_quantinc1y ann_sulfur_content10yr ann_sulfur_inc10yr ///
  ann_sulfur_inc5yr ann_sulfur_content5yr sulfur_content_6 sulfur_content_5 ///
  so2_unit reported_so2_lbs_mmbtu leg_lbs_mmbtu_calc
...
replace states=99 if states==33|states==35| states==42 | states==16 | states==15 | states==31 | states==9 | states==2| states==3 | states==37


** As we have only extremely few commercial boilers in our sample 
* and we use owner type FE and boiler FE, we merge commercial boilers into industrial
*C = Cooperative, I = Investor-Owned Utility, Q = Independent Power Producer, M = Municipally-Owned Utility, P = Political Subdivision, F = Federally-Owned Utility, S = State-Owned Utility, IND = Industrial, COM = Commercial
replace ut_type=5 if ut_type==2


** As we have only very few boilers of certain manufacturers
* and we use manufacturer FE and boiler FE in some regressions, 
* we merge these seldom manufacturers into one group
replace manufact=99 if manufact==1|manufact==2|manufact==5|manufact==6 |manufact==8|manufact==9|manufact==10|manufact==11|manufact==12|manufact==13|manufact==15|manufact==16|manufact==18|manufact==19|manufact==20|manufact==21


** Generate cutoff for local regulations removing GF privilege
* Assume that regulations requiring emissions less than 0.9 lbs/MMBtu
* effectively require scrubber
...
**************************************************
save "$path_use_data\regressions_ready_data.dta", replace
