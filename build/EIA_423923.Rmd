...
- `r path(l.path$data, "eia/f423923")`
...
download_file <- function(url) {
  ...
  ## Define destination
  file <- path(l.path$data, "eia/f423923", file)
...
if (length(l.url_dl)>length(fs::dir_ls(path(l.path$data, "eia/f423923"), regexp="\\.(xls|zip)$"))) {
...
extract_zip <- function(zip, file, sheet) {
  ...
  ## IMPORT DATA
  ## Unzip data
  unzip(zip, files=file, exdir=path(l.path$data, "eia/f423923"))
  
  ## Import data
  if (!sheet) {
    df <- readxl::read_excel(path(l.path$data, "eia/f423923", file), guess_max=10^5)
  } else {
    sheet <- readxl::excel_sheets(path(l.path$data, "eia/f423923", file)) %>%
      purrr::keep(str_detect(., "Page 5"))
    df <- readxl::read_excel(path(l.path$data, "eia/f423923", file), sheet=sheet, 
  ...
  file %>%
    ...
    purrr::flatten() %>%
    .[[1]] %>%
    fs::path(l.path$data, "eia/f423923", .) %>%
    fs::file_delete()
...
## Build zip list
l.zip <- fs::dir_ls(path(l.path$data, "eia/f423923"), regexp="f(4|9)23_\\d{4}\\.zip$")
