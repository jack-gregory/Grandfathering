...
This workbook forms a part of the data cleaning process related to the Grandfathering project.  It downloads and cleans annual sales by utility from the Energy Information Administration (EIA).  Specifically, it utilises the "Sales to Ultimate Customers" table reported in form EIA-860.  The data are available from 1990 onwards and represent a balanced panel by utility with the following variables:
...
download_zip <- function(url) {
  ...
  ## Define destination
  file <- path(l.path$data, "eia/f861", file)
...
if (length(l.zip_url)>length(fs::dir_ls(path(l.path$data, "eia/f861"), regexp="\\.zip$"))) {
  walk(l.zip_url, download_zip)
}
```
...
extract_zip <- function(zip, file) {
  ...
  ## IMPORT DATA
  ## Unzip data
  unzip(zip, files=file, exdir=path(l.path$data, "eia/f861"))
...
## Build zip list
l.zip_dir <- fs::dir_ls(path(l.path$data, "eia/f861"), regexp="f861_\\d{4}\\.zip$")

## Build zip & file dataframe
df.zip_dir <- l.zip_dir %>%
  map_dfr(~unzip(.x, list=TRUE) %>% mutate(Zip = .x)) %>%
  filter(str_detect(Name, "(TYP1|[Ff][Ii][Ll][Ee]2|Sales_Ult_Cust_\\d{4})")) %>%
