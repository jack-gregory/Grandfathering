...
l.rhs <- list(
  ...
  glue("Gf {ctrl_boiler} {ctrl_env} so2_nonat_Gf applic_reg_Gf {ctrl_sq} {fes}"),
...
## Construct column names
tbl_colnames <- df.reg %>%
  filter(name %in% c("Anticipation","Non-linear")) %>%
  distinct(name, lhs) %>%
  ...
  mutate(model_id = paste0("\\multicolumn{1}{c}{(", model_id, ")}"),
         name = ifelse(lhs=="Emissions", paste0("\\multicolumn{3}{c}{\\textit{", name, "}}"), ""),
         lhs = paste0("\\multicolumn{1}{c}{\\textit{", lhs, "}}")) %>%
  mutate_all(~ifelse(.!="", paste0(., " &"), "")) %>%
  add_row(model_id=" &", name=" &", lhs=" &", .before=1) %>%
  add_row(model_id="\\\\\\\\[-1.8ex]", name="\\\\\\\\[-1.8ex] \\cmidrule{2-4}\\cmidrule{5-7}", lhs="\\\\") %>%
  t() %>%
  as_tibble() %>%
  mutate_at(vars(last_col(offset=1)), ~str_replace(., " &$", ""))
...
tbl_header3 <- c("\\midrule \\\\",
                 "\\endhead")
tbl_midder <- c("\\hline \\\\ [-1.8ex]")
txt_footer <- c("This table presents robustness checks for anticipation and non-linear effects for our three channels, ",
                "namely: utilization, survival and emissions. ",
                "All regressions are based on our prefered specification: model (3) in Table (1). ",
                "The unit of observation is boiler-year, while . ",
                "The sample is restricted to commercial, industrial and IOU boilers as well as those boilers with , ",
                "inservice years between 1950 and 2006, inclusive. ",
                "All regressions include boiler controls as well as year, state, utility type and manufacture type fixed effects. ",
...
## Prepare file header, table header and footer, and subtable headers
f_header <- c("%% Grandfathering Project",
              "%% Alternative grandfathering regression table",
              glue::glue("%% Compiled on {base::Sys.time()} using <5_regs_robust.R>"),
              "%% Written by Jack Gregory\n")
tbl_header1 <- c("\\begin{center}",
                 "\\begin{singlespace}",
                 ...
                 paste0("\\begin{longtable}[c]{@{\\extracolsep{2.5ex}}l*{", 
                        ncol(tbl_colnames)-2, "}{D{.}{.}{-2}}@{\\extracolsep{2.5ex}}}"),
                 "\t\\caption{Alternative grandfathering regression results}",
...
tbl_header3 <- c("\\midrule \\\\",
                 "\\endhead")
tbl_midder <- c("\\hline \\\\ [-1.8ex]")
txt_footer <- c("This table presents robustness checks for grandfathered boilers with and without local regulations ",
                "for our three channels, namely: utilization, survival and emissions. ",
                "All regressions are based on our prefered specification: model (3) in Table (1). ",
                "The unit of observation is boiler-year, while . ",
                "The sample is restricted to commercial, industrial and IOU boilers as well as those boilers with , ",
                "inservice years between 1950 and 2006, inclusive. ",
