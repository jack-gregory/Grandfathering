...
l.rhs <- list(
  ...
  # glue("{ctrl_boiler} {ctrl_env} {ctrl_plb} {fes}"),
...
## Construct table summary
l.sum <- list(#"fe_yr","fe_st","fe_ut","fe_man","ctrl_mkt","ctrl_so2",
              "N","r2")
l.sum_labs <- list(#"Year FE","State FE","Utility FE","Manufacture FE","Market Controls","Sulfur Controls",
                   "Observations","R$^{2}$")
tbl_summary <- df.reg %>%
  filter(name=="Non-linear Age" | (name=="Spillover" & lhs=="DURATION")
         ) %>%
...
## Output TeX table file
file <- here::here(l.path$out, date, "tbl.reg_anticipation.tex")
f_header %>% write_lines(file, append=FALSE)
