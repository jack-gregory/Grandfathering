...
l.file <- list(
  ...
  gf = path(l.path$data, "gf_original/regression_vars.csv"),
  ...
  rail = path(l.path$data, "bts/North_American_Rail_Network_Lines.zip"),
  rail1 = path(l.path$data, "bts/North_American_Rail_Network_Lines_-_Class_I_Freight_Railroads_View.zip"),
  marine = path(l.path$data, "bts/Marine_Highways.zip"),
...
<!-- , to only Class I railways, and to the top seven rail freight firms which account for around 94% of rail freight revenues (Preonas, 2023). -->
...
bts_rail_files <- zip::zip_list(l.file$rail) %>%
  pull(filename)
zip::unzip(l.file$rail, file=bts_rail_files, exdir=path(l.path$data, "bts"))

## Import rail shapefile
df.rail <- fs::path(l.path$data, "bts") %>%
  fs::dir_ls(regexp="\\.shp$") %>%
  read_sf() %>%
  filter(country=="US") %>%
  filter(!(stateab %in% c("AK","HI"))) %>%
  filter(stfips!="99") #%>%
  # filter(RROWNER1 %in% c("BNSF","CN","CPRS","CSXT","KCS","NS","UP"))

## Remove BTS rail network data
purrr::map(
  bts_rail_files,
  ~fs::file_delete(fs::path(l.path$data, "bts", .x))
)
...
bts_marine_files <- zip::zip_list(l.file$marine) %>%
  pull(filename)
zip::unzip(l.file$marine, file=bts_marine_files, exdir=path(l.path$data, "bts"))

## Import marine shapefile
df.marine <- fs::path(l.path$data, "bts") %>%
  fs::dir_ls(regexp="\\.shp$")  %>%
  read_sf() %>%
  filter(!(st_usps %in% c("AK","HI","PR","XX"))) %>%
  filter(!is.na(st_usps)) #%>%
  # filter(!(ww_name %in% c("Gulf Intracoastal Waterway"))) %>%
  # filter(!stringr::str_detect(ww_name, "Deep Water Access"))
...
ggplot() +
  ...
  labs(title="Sample plants, mines and networks",
       colour="") +
  theme_maps() #+
  # guides(colour = guide_legend(override.aes=list(size=2.5, linewidth=1.2,
...
#   st_coordinates()
# 
# ## ... cluster nodes using the DBSCAN spatial clustering algorithm
# ## NB - We set eps = 0.5 such that nodes within a distance of 0.5 from each other will be in the same cluster.
# ##      We set minPts = 1 such that a node is assigned a cluster even if it is the only member of that cluster.
# library(dbscan)
# clusters = dbscan(net_coords, eps=1e-1, minPts=1)$cluster
# 
# ## ... add cluster information to the nodes of the network.
# net <- net |>
#   activate("nodes") |>
#   mutate(cls = clusters,
...
# net <- net  |>
#   tidygraph::convert(to_spatial_contracted, cls, cmp, simplify=TRUE) |>
#   # tidygraph::convert(to_spatial_smooth, .clean=TRUE) |>
#   activate("nodes") |>
#   mutate(ID = row_number())
# net
...
ggplot() +
  ...
  # geom_sf_text(
  #   data=st_as_sf(net_all, "nodes") |> mutate(ID = row_number()),
  #   aes(label=ID),
  #   size=3, hjust=-0.5
  # ) +
  geom_sf(data=st_intersection(df.plant, st_buffer(df.marine_sample, 2e4)),
  ...
  #       stat="sf_coordinates", min.segment.length=0, size=3, hjust=-0.5) +
  # geom_sf_text(
  #   data=st_intersection(df.sulfur_cnty, st_buffer(df.marine_sample, 2e4)),
  #   aes(label=FID),
  #   size=3, hjust=0
  # ) +
  scale_color_manual(values=c("black","grey40","gold3")) +
...
`r text_colour(colour="red", text="NB - Consider removing unconnected rail sections.  See assessment plots below.")`
...
`r text_colour(colour="red", text="NB - Consider fixing unlinked marine highways.  See assessment plots below.")`
...
ggplot() +
  ...
  #   size=3, hjust=-0.5
  # ) +
  # geom_sf(data=st_intersection(df.plant, st_buffer(net, 2e4)),
  #         aes(color="Plant"), size=2.0, shape=1) +
  # geom_sf_text(
  #   data=st_intersection(df.plant, st_buffer(net, 2e4)),
  #   aes(label=ORISPL),
  #   size=3, hjust=0
  # ) +
  # geom_sf(data=st_intersection(df.sulfur_cnty, st_buffer(net, 2e4)),
  #         aes(color="Coal"), size=2.0, shape=1) +
  # geom_text_repel(data=st_as_sf(net, "nodes"), aes(label=ID, geometry=geometry),
  #       stat="sf_coordinates", min.segment.length=0, size=3, hjust=-0.5) +
  # geom_sf_text(
  #   data=st_intersection(df.sulfur_cnty, st_buffer(df.marine_sample, 2e4)),
...
- Weighted average of county sulfur medians by inverse network distance; and,
- Weighted average of county sulfur medians by inverse square network distance.
...
Second, we calculate network distances between all plants and county centroids.  We use these distances to generate the last two metrics described above based on network distances.
...
df.distance_net <- st_network_cost(
  ...
  left_join(st_as_sf(net_all, "nodes") |>
              ...
              select(SULFUR_ID, FID, SULFUR_MEDIAN), 
            by=c("SULFUR_ID")) |>
...
df.distance <- df.distance_med %>%
  ...
  full_join(df.distance_net, by=c("ORISPL")) %>%
...
map2(list(expr(SULFUR_DIST), expr(SULFUR_DIST2), expr(SULFUR_DISTLOW),
          expr(SULFUR_NET), expr(SULFUR_NET2)),
